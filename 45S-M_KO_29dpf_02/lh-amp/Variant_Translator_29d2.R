...
#import sample info
samples <- read.csv("45S-M_KO_29dpf_02.csv", header=TRUE) # list includes the PBAT samples not relevant to this script
...
samples$ID <- gsub("_L001_R1.fastq", "", samples$ID) # the Filename structure was different between iSeq runs

samples <- samples[grep("lh-amp", samples$protocol), ] # remove PBAT files from list
samples <- samples[grep("_R1", samples$filename), ]
df <- inner_join(df,samples,by="ID") #should not in/decrease obs beyond those in df
...
ggplot(subset(ref_con), aes(x = Individual, y = ref_con,fill = treatment))+ #, fill = treatment
  ...
  theme(legend.position = "null",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
