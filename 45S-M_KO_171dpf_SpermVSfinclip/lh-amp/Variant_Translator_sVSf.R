...
#import sample info
samples <- read.csv("45S-M_KO_171dpf_spermVSfinclip.csv", header=TRUE) # list includes the PBAT samples not relevant to this script
...
df$sample<- gsub("_L001_pr_on_rl_Output.txt", "", df$Source)
samples$sample <- gsub("_L001_R1_001.fastq", "", samples$filename)
samples <- samples[grep("lh-amp", samples$protocol), ] # remove PBAT files from list
samples <- samples[grep("_R1", samples$filename), ]

df <- inner_join(df,samples,by="sample") #should not increase obs beyond those in df
df$sample <- gsub("_.*", "", df$sample) #for uniting primary and re-seq reads
...
df <- df[df$sample %in% factors_to_keep, ]



# calculate variant proportions /sample
casVARprop <- df %>%  group_by(casVAR,treatment,Individual,tissue,sample) %>%  summarize(Count = n()) %>%  group_by(sample) %>%  mutate(Proportion = Count / sum(Count) * 100) %>%  ungroup()
...
ref_con <- left_join(ref_con,preads,by=c("sample"))
...
ggplot(ref_con, aes(x = Individual, y = ref_con,fill = treatment))+ #, fill = treatment
  geom_col()+
  ylim(0,100)+
...
ggplot(casVARprop, aes(x = Individual, y = Proportion, fill = casVAR))+
  ...
  facet_grid(cols = vars(treatment),rows = vars(tissue),scales = "free", space = "free")+
...
#### Loss of diversity???  aka shannon entropy
entro <- casVARprop
samplelist <- unique(entro$sample)
calculate_entropy <- function(sample_data) {# Function to calculate Shannon entropy
  proportions <- sample_data$Proportion / sum(sample_data$Proportion)
  entropy <- -sum(proportions * log2(proportions), na.rm = TRUE)
  return(entropy)
}
entropy_values <- sapply(samplelist, function(samEple) { # Calculate Shannon entropy for each sample
  sample_data <- subset(entro, sample == samEple)
  entropy <- calculate_entropy(sample_data)
  return(entropy)
})

# Create a new dataframe with sample and entropy values
entropy <- data.frame(sample = samplelist, ShannonEntropy = entropy_values, row.names = NULL)
entropy <- entropy %>%  separate(sample, into = c("Tissue","Sire"), sep = "-", remove = TRUE)
entropy$Sire<- gsub("Sire", "", entropy$Sire)
entropy$trtmnt <- ifelse(as.numeric(gsub("#", "", entropy$Sire)) <= 21, "Inj.", "Uni.")

#plot it
ggplot(entropy, aes(x = trtmnt, y = ShannonEntropy, fill = Tissue)) +
  geom_boxplot()+
  labs(fill = "")+
  scale_fill_manual(values=c("#C4D4E0","#F2EFDE"))+
  theme_classic()+
  #stat_compare_means(comparisons = my_comparisons, label = "p.signif",vjust = 0.7)+ (wilcoxon> Inj *, Uni ns ; IvsU ****)
  facet_grid(cols = vars(trtmnt), scales = "free_x", space = "free_x",switch="both")+
  theme(legend.position = "bottom",axis.title.x = element_blank(),legend.text = element_text(margin = margin(t = 0, r = 0, b = 0, l = -5)),
        legend.key.size = unit(0.4, "cm"), legend.key.width = unit(0.3, "cm"),
        #plot.margin = margin(t = 0, r = 0, b = 0, l = 0, unit = "cm"),
        legend.margin = margin(b = 0.0002,t = 0.75, unit = "cm"))
