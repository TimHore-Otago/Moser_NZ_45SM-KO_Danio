...
#import sample info
samples <- read.csv("45S-M_KO_100dpf.csv", header=TRUE) # list includes the PBAT samples not relevant to this script
...
factors_to_keep <- names(sample_counts[sample_counts > 150])
...
ref_con$MOE<- (2.326348*sqrt(((ref_con$ref_con/100)*(1-(ref_con$ref_con/100)))/ref_con$sum_readprs_cVc))*100 #--> 99% confidence interval
...
# samples with >5% margin of error at a 99% confidence interval are ignored
ggplot(subset(ref_con, MOE<5), aes(x = Individual, y = ref_con,fill = treatment))+ #, fill = treatment
  geom_col()+
  ylim(0,100)+
  theme(legend.position = "bottom",axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))


# Plotting all indel variants (no MOE Filter!!)
ggplot(casVARprop, aes(x = Individual, y = Proportion, fill = casVAR))+
  ...
  theme(legend.position = "bottom", legend.text = element_text(margin = margin(t = 0, r = 0, b = 0, l = -5)),
        legend.key.size = unit(0.4, "cm"), legend.key.width = unit(0.3, "cm"),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)
        ) +
